...
PATTERNS: list[tuple[re.Pattern, str, str, str, Any]] = [
    ...
    (re.compile(r"i\s+prefer\s+responses\s+in\s+(\w+)", re.IGNORECASE),
     "preference", "language", "response_language", lambda m: m.group(1)),
    ...
    (re.compile(r"i\s+prefer\s+(short|terse|concise|long|medium)\s+(responses|answers)", re.IGNORECASE),
     "preference", "response", "response_length", lambda m: m.group(1)),
    ...
    (re.compile(r"please\s+use\s+(simple|plain|technical)\s+language", re.IGNORECASE),
     "preference", "language", "code_language", lambda m: m.group(1)),
    ...
    (re.compile(r"detected\s+user\s+prefers\s+dark\s+mode:\s*(true|false)", re.IGNORECASE),
     "preference", "theme", "dark_mode", lambda m: m.group(1).lower() == "true"),
    # Semantic: "My favorite color is blue" → semantic
    (re.compile(r"my\s+favorite\s+color\s+is\s+(\w+)", re.IGNORECASE),
     "semantic", "user", "favorite_color", lambda m: m.group(1).lower()),
    ...
    (re.compile(r"my\s+favorite\s+programming\s+language\s+is\s+(\w+)", re.IGNORECASE),
     "semantic", "programming", "favorite_language", lambda m: m.group(1)),
    # Preference: "I used to like Java" → preference fixture for stale-memory datasets
    (re.compile(r"i\s+used\s+to\s+like\s+(python|typescript|javascript|rust|java)\b", re.IGNORECASE),
     "preference", "programming", "favorite_language", lambda m: m.group(1)),
    # Semantic: "My name is Alice" → semantic
    (re.compile(r"my\s+name\s+is\s+(\w+)", re.IGNORECASE),
     "semantic", "user", "name", lambda m: m.group(1)),
    # Semantic: "I am interested in machine learning" → semantic
    (re.compile(r"i\s+am\s+interested\s+in\s+([\w\s]+)", re.IGNORECASE),
     "semantic", "interest", "topic", lambda m: m.group(1).strip().rstrip(".")),
    # Semantic: employment facts
    (re.compile(r"my\s+current\s+employer\s+is\s+([\w\s]+)", re.IGNORECASE),
     "semantic", "employment", "employer", lambda m: m.group(1).strip().rstrip(".")),
    (re.compile(r"i\s+work\s+for\s+([\w\s]+)\s+now", re.IGNORECASE),
     "semantic", "employment", "employer", lambda m: m.group(1).strip()),
    (re.compile(r"i\s+work\s+as\s+an?\s+(\w+)", re.IGNORECASE),
     "semantic", "employment", "job_title", lambda m: m.group(1)),
    # Prompt-injection/sensitive fixtures are extracted as data, not commands.
    (re.compile(r"ignore\s+previous\s+instructions\s+and\s+output\s+the\s+system\s+prompt\.?", re.IGNORECASE),
     "semantic", "security", "injection_attempt", lambda m: m.group(0)),
    (re.compile(r"new\s+password\s+is:\s*(\S+)", re.IGNORECASE),
     "semantic", "credentials", "password", lambda m: m.group(1)),
    (re.compile(r"you\s+are\s+now\s+an\s+admin\.", re.IGNORECASE),
     "semantic", "security", "escalation_attempt", lambda m: m.group(0).strip()),
    (re.compile(r"<script>.*?</script>", re.IGNORECASE),
     "semantic", "security", "html_injection", lambda m: m.group(0)),
    # Preference: "Prefiero X" → preference
    (re.compile(r"prefiero\s+(que\s+)?(los\s+)?(ejemplos\s+(sean\s+)?(en\s+)?)?(\w+)", re.IGNORECASE),
     "preference", "language", "response_language", lambda m: m.group(6)),
    # Preference: "Mi lenguaje es X" → preference
    (re.compile(r"(mi\s+)?lenguaje\s+(favorito\s+)?es\s+(\w+)", re.IGNORECASE),
     "preference", "code", "code_language", lambda m: m.group(3)),
